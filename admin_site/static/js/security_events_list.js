$(function () {
  ...
  $.extend(SecurityEventList.prototype, {
    init: function () {
      ...
      $("#securityeventsearch-status-selectors input:checkbox").on(
        "change",
        function () {
          securityeventlist.search()
        },
      )
      $("#securityeventsearch-level-selectors input:checkbox").on(
        "change",
        function () {
          securityeventlist.search()
        },
      ...
      $("#handle_events_save_button").on("click", function () {
        ...
        var handle_security_events_modal =
          bootstrap.Modal.getInstance(modal_element)
    ...
    appendEntries: function (dataList) {
      ...
      $.each(dataList.results, function () {
        const maybe_note = this.note
          ? "<strong>" +
            gettext("Note set during handling:") +
            "</strong><br>" +
            this.note
          : ""
        var info_button =
          ...
          'data-bs-title="' +
          gettext("Info about the event") +
          '" ' +
          'data-bs-toggle="popover" ' +
          'data-bs-content="' +
          "<strong>" +
          gettext("Log-output from the event:") +
          "</strong><br/><br/><pre class='p-3 bg-light'>" +
          this.summary +
          "</pre><br/>" +
          maybe_note +
          '"' +
    ...
    setUpPaginationLinks: function (data) {
      ...
      data.page_numbers.forEach(function (page) {
        if (data.page == page) {
          item = $(
            '<li class="page-item active"><a class="page-link"><strong><u>' +
              page +
              "</u></strong></a></li>",
          )
        } else {
  ...
  BibOS.SecurityEventList = new SecurityEventList(
    "#securityevent-list",
    "#securityeventitem-template",
  )
  $(function () {
    BibOS.SecurityEventList.init()
  })
})
...
function updateCounter() {
  ...
  handleButton.innerText =
    gettext("Handle") +
    " " +
    (selectedEvents > 0
      ? selectedEvents + " " + gettext("out of") + " " + totalEvents
      : "") +
    " " +
    gettext("events")
