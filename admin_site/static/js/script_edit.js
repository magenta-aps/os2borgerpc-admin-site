;(function (BibOS, $) {
  ...
  $.extend(ScriptEdit.prototype, {
    ...
    addInput: function (id, data_in) {
      ...
      var elem = $(BibOS.expandTemplate("script-input", data))
      elem.find("input.name-input")[0].required = true
      elem.find("input.mandatory-input")[0].checked =
        elem.find("input.mandatory-input")[0].value != "false"
      elem.find("select").val(data["value_type"])
      if (
        elem.find("select")[0].value == "BOOLEAN" ||
        elem.find("select")[0].value == "TEXT_FIELD"
      ) {
    ...
    updateInputNames: function (id) {
      ...
      inputs.each(function (i, e) {
        ...
        elem
          ...
          .attr("name", "script-input-" + i + "-name")
        elem
          .find("select.type-input")
          .attr("name", "script-input-" + i + "-type")
        elem
          .find("input.default-input")
          .attr("name", "script-input-" + i + "-default")
        elem
          .find("input.mandatory-input")
          .attr("name", "script-input-" + i + "-mandatory")
      })
